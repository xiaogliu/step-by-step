...
exports.post = async function(ctx, next) {
  try {
    ...
    const connection = connectionModel.getConnection();
    const query = bluebird.promisify(connection.query.bind(connection));
    const posts = await query(`select * from post where id = "${id}"`);
    let post = posts[0];

    const comments = await query(
      `select comment.*,user.username from comment left join user on comment.userId = user.id where postId = "${
        post.id
...
exports.addComment = async function(ctx, next) {
  try {
    ...
    const result = await query(
      `insert into comment(userId,postId,content,createdAt) values("${ctx.cookies.get(
        "userId"
      )}", "${data.postId}", "${data.content}",${connection.escape(
        new Date()
      )})`
    );
    if (result) {
