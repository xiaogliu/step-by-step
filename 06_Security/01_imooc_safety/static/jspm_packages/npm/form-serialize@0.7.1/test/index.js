...
test('single element', function() {
  ...
  hash_check(form, {'foo': 'bar'});
  str_check(form, 'foo=bar');
  empty_check(form, 'foo=bar');
  empty_check_hash(form, {'foo': 'bar'});
});
...
test('radio - single default', function() {
  ...
  hash_check(form, {foo: 'bar1'});
  str_check(form, 'foo=bar1');
  empty_check(form, 'foo=bar1');
  empty_check_hash(form, {foo: 'bar1'});
});
test('radio w/checkbox', function() {
  ...
  hash_check(form, {foo: ['bar3', 'bar1', 'bar5']});
...
test('bracket notation - hashes with a digit as the first symbol in a key', function() {
  ...
  hash_check(form, {'somekey': {'123abc': {
        'first': 'first_value',
        'second': 'second_value'
      }}});
  empty_check_hash(form, {'somekey': {'123abc': {
        'first': 'first_value',
        'second': 'second_value'
      }}});
});
...
test('bracket notation - select multiple, nested', function() {
  ...
  hash_check(form, {foo: {bar: ['baz', 'norf']}});
});
test('bracket notation - select multiple, empty values', function() {
  ...
  hash_check(form, {foo: {bar: ['Default value', 'baz', 'norf']}});
  empty_check_hash(form, {foo: {bar: ['Default value', '', 'baz', 'norf']}});
});
test('bracket notation - non-indexed arrays', function() {
  ...
  hash_check(form, {people: [{name: "fred"}, {name: "bob"}, {name: "bubba"}]});
});
test('bracket notation - nested, non-indexed arrays', function() {
  ...
  hash_check(form, {user: {tags: ["cow", "milk"]}});
});
...
test('custom serializer', function() {
  ...
  assert.deepEqual(serialize(form, {serializer: function(curry, k, v) {
      curry[k] = 'ZUUL';
      return curry;
    }}), {"node": "ZUUL"});
});
