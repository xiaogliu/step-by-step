...
这是 从生成器到 Promise 再到 async-await 的第二部分，第一部分介绍了生成器，这部分介绍 Promise，第三部分介绍 async-await。
...
这也是 “promise” 名称的由来：承诺将来会给一个值，这个值可能成功的数据也可能错误信息，总之，会给一个值。
...
- 第一个参数为称为 success 回调函数，在 promise 执行`resolve`函数后执行，同时可以接收`resovle`的返回值作为参数；
- 第二个参数称为 failure 回调函数，在 promise 执行`reject`函数或者程序发生异常时执行，可以接收`reject`的返回值或者错误信息作为参数。见下面例子（接下来的例子就不把 executor 函数单独提出来了）：
...
这里要注意，`then()`方法里面的函数是异步执行的，**最快在当前事件循环的微任务开始执行时执行**，这里有两点要注意：
...
上面代码中，我们在 executor 函数中直接执行了`resolve`，所以后面调用`then()`方法的时候执行了`success`函数，同时将`resolve`的返回值`111`传入`success`函数。现在我们看下打印顺序：
...
> 为了分离错误处理的代码，promise 还提供了 catch 方法，后面讨论，这里先介绍基本用法。
...
Promise 作为异步数据的占位符，在整个声明周期中会有不同状态，这些状态在其内部属性中都有体现。

## 2.1 Promise 状态

Promise 整个生命周期状态可分为两个部分：未完成状态和已完成状态，其中，未完成状态只有 pending 一种情况，而已完成状态又分为已兑现（fullfilled，`resolve`函数执行）和被拒绝（rejected，`reject`函数执行或者 promise 处理时发生异常）两种情况，见下图：
...
promiseAjax("URL1")
  .then(data1 => {
    console.log(data1);
    promiseAjax(data1[0].url2);
  })
  .then(data2 => {
    console.log(data2);
    promiseAjax(data2[0].url3);
  })
  .then(console.log(data3))
  .catch(e => console.log(e));
```

链式调用之所以能够实现是因为 **`then()`返回的也是 promise 对象**，其中任何一个请求出现错误，都会被`catch()`捕捉到。
